...
class SinaUrl{
    ...
    public static function getShort($url){
        $url='http://api.t.sina.com.cn/short_url/shorten.json?source='.self::APPKEY.'&url_long='.urlencode($url);
        $result=self::CURLQueryString($url);
        return self::doWithResult($result,'url_short');
    }

    public static function getBatchShort($srcUrl){
        $tempUrl = [];
        foreach($srcUrl as $val){
            $tempUrl[] = 'url_long='.urlencode($val);
        }
        $paramUrl = implode('&', $tempUrl);
        $url='http://api.t.sina.com.cn/short_url/shorten.json?source='.self::APPKEY.'&'.$paramUrl;
        $result=self::CURLQueryString($url);
        $result=json_decode($result,true);
        $newUrl = [];
        foreach($result as $val){
            $newUrl[] = $val['url_short'];
        }
        return $newUrl;
    }
...
//使用示例，如下：
//$result=SinaUrl::getShort('http://www.sohu.com/');
//echo $result;
